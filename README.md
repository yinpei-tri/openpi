...
We provide example fine-tuning configs for [π₀](src/openpi/training/config.py), [π₀-FAST](src/openpi/training/config.py), and [π₀.₅](src/openpi/training/config.py) on LIBERO data. There is also a `pi05_libero_debug` config — the same data + model as `pi05_libero` but with `batch_size=4`, `num_train_steps=200`, and EMA off — designed to fit on two 48 GB GPUs (e.g. A6000) for a quick forward+backward smoke run. It reuses `pi05_libero`'s norm stats via `AssetsConfig`, so you only need to run `compute_norm_stats` once.
...
Defaults to `TRAIN_CONFIG=pi05_libero_debug`. Override `TRAIN_CONFIG`, `EXP_NAME`, `TRAIN_ARGS`, or replace the entrypoint via `TRAIN_CMD` for the PyTorch trainer. See [scripts/docker/RUNBOOK.md](scripts/docker/RUNBOOK.md) for the full workflow including the DGX A100 recipe.
...
The PyTorch trainer mirrors the JAX trainer's EMA behaviour:

- **EMA** (exponential moving average) — driven by `config.ema_decay`. Enabled when set (e.g. `0.999` on `pi05_libero`); set `ema_decay=None` to disable. The shadow is kept in float32 and saved alongside each checkpoint as `ema.safetensors`. On resume, the shadow is restored from disk; if it's missing (e.g. resuming from a JAX-converted checkpoint) it is re-initialized from the current weights.
...
Each checkpoint directory `<exp>/<step>/` contains `model.safetensors`, `optimizer.pt`, `metadata.pt`, optional `ema.safetensors` (when `ema_decay is not None`), and the dataset's `assets/<asset_id>/` so inference can run from the checkpoint alone.

If you converted a JAX checkpoint via `examples/convert_jax_model_to_pytorch.py` and want to start finetuning from it, set `pytorch_weight_path` in your config rather than passing `--resume` — `--resume` expects a full `<step>/` directory layout, while `pytorch_weight_path` just loads `model.safetensors` and starts the optimizer / LR schedule fresh.
