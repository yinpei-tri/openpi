...
if [[ "${1:-}" == "--wait" ]]; then
  ...
  echo "[release] waiting for combined_eval clients to drain..."
  empty=0
  while :; do
    n=$(pgrep -cf "[c]ombined_eval.py" || true)
    if [[ "${n:-0}" -gt 0 ]]; then
      empty=0
    else
      empty=$((empty + 1))
...
pkill -f "[c]ombined_eval.py"     2>/dev/null && echo "[release] killed rollout clients" || true
