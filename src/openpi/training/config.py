...
_CONFIGS = [
    ...
    #
    # System1 EVAL-ONLY serving configs. One per trained checkpoint variant (all share the
    # base pi05_robocasa_system1 data recipe: anchor images+state, task_goal, conditioning,
    # gripper flag). These exist so `serve_policy.py --policy.config <name>` reconstructs the
    # EXACT model architecture the checkpoint was trained with (progress head vs
    # progress-as-action) — the training runs set these via CLI overrides on the base config,
    # so a registered config is needed to serve them faithfully. Data/loss knobs that only
    # matter at train time are irrelevant here; only model shape + data-transform recipe do.
    #
    # (1) 10-class progress classifier (prog-classes). Same as the base config; registered
    #     under an eval name for symmetry with the other two.
    TrainConfig(
        name="pi05_robocasa_system1_eval_classes",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=20,
            max_token_len=256,
        ...
        batch_size=64,
        num_workers=0,
        lr_schedule=_optimizer.CosineDecaySchedule(warmup_steps=500, peak_lr=5e-5, decay_steps=20_000, decay_lr=5e-5),
        optimizer=_optimizer.AdamW(clip_gradient_norm=1.0),
        ema_decay=0.999,
        weight_loader=weight_loaders.CheckpointWeightLoader(
            "gs://openpi-assets/checkpoints/pi05_base/params",
    ...
    # (2) Progress-as-action (progact): NO progress head; the flow head predicts a 12th
    #     action dim = per-step progress. SplitProgressAction (wired by progress_as_action)
    #     pops it out to a `progress` key in the infer() output. use_progress_head=False.
    TrainConfig(
        name="pi05_robocasa_system1_eval_progact",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=20,
            max_token_len=256,
    ...
    # (3) Continuous (regression) progress head (prog-continuous, loss weight 1). Scalar
    #     sigmoid state-value read via predict_progress (not in the stock infer() output).
    TrainConfig(
        name="pi05_robocasa_system1_eval_reg",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=20,
            max_token_len=256,
