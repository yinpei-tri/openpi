...
def save_checkpoint(model, optimizer, global_step, config, is_main, data_config, ema=None):
    ...
    if (global_step % config.save_interval == 0 and global_step > 0) or global_step == config.num_train_steps - 1:
        ...
        # Save EMA shadow alongside the live weights when EMA is enabled.
        if ema is not None:
...
def load_checkpoint(model, optimizer, checkpoint_dir, device, ema=None):
    """Load the latest checkpoint and return (global_step, ema_loaded)."""
    checkpoint_steps = [
        int(d.name)
...
def train_loop(config: _config.TrainConfig):
    ...
    # EMA shadow (mirrors the JAX trainer's EMA-on-optimizer-state behaviour).
    ema = None
    ...
    global_step = 0
    if resuming:
        global_step, ema_loaded = load_checkpoint(model, optim, config.checkpoint_dir, device, ema=ema)
        if ema is not None and not ema_loaded:
