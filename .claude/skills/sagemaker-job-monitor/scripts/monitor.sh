...
analyze_logs() {
  ...
  # Print health signals per node stream. MATCH env pins which streams belong to this job by
  # a string present in the LOG BODY (skips other users' recent streams); it DEFAULTS to the
  # job substring the caller passed (usually the timestamp, which appears in the checkpoint
  # S3 URI the entrypoint prints), so single- and multi-node RoboCasa/LIBERO runs all match
  # without hardcoding an exp_name. NODES caps how many streams to show (default 2 covers a
  # 2-node run; single-node has 1 and the loop just stops early). STREAM_HASH pins the exact
  # AWSBatch<hash> prefix if known (then MATCH is skipped — the stream already identifies the job).
  local match="${MATCH:-$JOB}"
  local nodes="${NODES:-2}"
  local shown=0
  for s in $streams; do
    [ $shown -ge "$nodes" ] && break
    local msgs
    ...
    shown=$((shown+1))
    echo "########## $s ##########"
    # Topology: single-node logs "jax process 0/1 ... global devices 8"; multi-node "0/2 ... 16".
    echo "$msgs" | grep -aE "Running on:|global devices" | tail -1
    # MULTI-NODE ONLY signals (absent/uninformative single-node — printed if present):
    #   per-shard image means: proc=0 vs proc=1 MUST differ (else both nodes read same data)
    #   data-shard-split: want jax_proc=N/2 (N/1 = worker didn't see the process count)
    echo "$msgs" | grep -aE "per-shard image means|data-shard-split" | tail -2
    ...
    echo "$msgs" | grep -aE "Traceback|Shutdown barrier|Aborted|core dumped|Error occurred|NaN|non-trivial shardings|final checkpoint upload failed" | tail -3
    echo ""
  done
}
